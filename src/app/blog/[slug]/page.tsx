...
async function getRelated(category: string, excludeSlug: string) {
  try {
    ...
    return posts.filter((p: any) => p.published && p.category === category && p.slug !== excludeSlug).slice(0, 3);
  } catch { return []; }
}
  ...
  return (
    <>
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="bg-bg-light rounded-2xl p-8 border-l-4 border-primary mb-12">
          <p className="text-xl text-slate-700 leading-relaxed italic">{post.excerpt}</p>
        </div>
        ...
        {!content.sections && !content.introduction && <div className="text-center py-20 text-slate-400"><span className="material-symbols-outlined text-6xl mb-4 block">article</span><p className="text-lg">Le contenu détaillé de cet article sera bientôt disponible.</p></div>}
        ...
        <section className="bg-bg-light py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((r: any) => (
                <Link key={r.id} href={`/blog/${r.slug}`} className="group bg-white rounded-xl p-5 border border-slate-100 hover:shadow-lg hover:border-primary/20 transition-all">
                  <span className="text-xs font-bold text-primary bg-primary/10 px-2 py-1 rounded-full">{r.category}</span>
                  <h3 className="font-bold text-slate-900 mt-3 mb-2 group-hover:text-primary transition-colors line-clamp-2">{r.title}</h3>
